...
def read_snli_files(filetype):
    ...
    fn = os.path.join('.data/snli/snli_1.0_{}.jsonl'.format(split))
    with open(fn) as f:
...
def split_snli_files():
    filename = "temp/split_snli_files" 
    if os.path.exists(filename):
        ...
        train_perms, train_hypos, train_labels = read_snli_files('train')
        dev_perms, dev_hypos, dev_labels = read_snli_files('dev')
        test_perms, test_hypos, test_labels = read_snli_files('test')
        f=open(filename,'wb')
        saved={}
...
def split_imdb_files():
    filename = "temp/split_imdb_files" 
    if os.path.exists(filename):
