...
def loadData(opt,embedding=True):
    ...
    if opt.data_from_file is not None:
        filename= opt.data_from_file
        f=open(filename,'rb')
        saved=pickle.load(f)
        f.close()
        datas = saved['datas']
        opt.embeddings=saved["embeddings"]
        opt.alphabet=saved['alphabet']
        opt.embedding_dim=saved['embedding_dim']
        ...
        #    opt.label_size= len(label_alphabet)
            ...
            if opt.model=="bert": 
                ...
                alphabet,tokenizer = load_vocab_for_bert_selfmade()
        ...
        for data in datas:
            if "bert" not in opt.model.lower():
                ...
                if opt.model=="bert": 
                    ...
                    data["text"]= data["text"].apply(process_with_bert_selfmade,tokenizer=tokenizer,max_seq_len = opt.max_seq_len)
            data["label"]=data["label"].apply(lambda text: label_alphabet.get(text)) 
        

        filename= "temp/%s_%s_all" % (opt.dataset, opt.model)
        f=open(filename,'wb')
        save={'datas':datas,'embeddings':torch.FloatTensor(vectors),'alphabet':alphabet,'embedding_dim':vectors.shape[-1],'vocab_size':len(alphabet),'label_size':len(label_alphabet) }
        pickle.dump(save,f)
        f.close()
