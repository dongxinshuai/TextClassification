...
def train(opt,train_iter, dev_iter, test_iter, syn_data, verbose=True):
    ...
    best_adv_acc = 0
    for epoch in range(opt.snli_epochs):
        ...
        for iters,batch in enumerate(train_iter):
            ...
            if opt.pert_set=="ad_text_syn_p":
                ...
                adv_x_p_comb_p, adv_x_h_comb_p = model(mode="get_adv_by_convex_syn", x_p=embd_p, x_h=embd_h, label=label, 
                ...
                temp = x_h_text_like_syn_valid.sum(dim=-1)
                nsw = (temp==1).sum()
                sc += (temp.sum()-nsw)
                ...
                adv_x_p, adv_x_h = model(mode="get_adv_hotflip", x_p=x_p, x_h=x_h, label=label, 
                        x_p_text_like_syn=x_p_text_like_syn, x_p_text_like_syn_valid=x_p_text_like_syn_valid, 
                        x_h_text_like_syn=x_h_text_like_syn, x_h_text_like_syn_valid=x_h_text_like_syn_valid,
                        x_p_mask=x_p_mask, x_h_mask=x_h_mask,
                        attack_type_dict=attack_type_dict)
                ...
                embd_p, embd_h = model(mode="text_to_embd", x_p=x_p, x_h=x_h) #in bs, len sent, vocab
                embd_p_adv, embd_h_adv = model(mode="get_embd_adv", x_p=embd_p, x_h=embd_h, label=label, 
                    x_p_mask=x_p_mask, x_h_mask=x_h_mask,
                    attack_type_dict=attack_type_dict)
            ...
            # adv loss
            if opt.pert_set=="ad_text_syn_p":
                #predicted_adv = model(mode="text_syn_p_to_logit", x_p=x_p_text_like_syn, x_h=x_h_text_like_syn,x_p_comb_p=adv_x_p_comb_p, x_h_comb_p=adv_x_h_comb_p )
                predicted_adv = model(mode="text_syn_p_to_logit_hypo_only", x_p=x_p, x_h=x_h_text_like_syn, x_h_comb_p=adv_x_h_comb_p, x_p_mask=x_p_mask, x_h_mask=x_h_mask)
            elif opt.pert_set=="l2_ball":
                predicted_adv = model(mode="embd_to_logit", x_p=embd_p, x_h=embd_h_adv, x_p_mask=x_p_mask, x_h_mask=x_h_mask)
            elif opt.pert_set=="ad_text_hotflip":
                predicted_adv = model(mode="text_to_logit", x_p=adv_x_p, x_h=adv_x_h, x_p_mask=x_p_mask, x_h_mask=x_h_mask)
            ...
            # optimize
            loss =  opt.weight_kl * kl_control * loss_kl + opt.weight_adv * loss_adv + opt.weight_clean * loss_clean
            loss.backward() 
            optimizer.step()
            sum_loss += loss.item()
        ...
        if epoch%5==0:
            ...
            adv_acc = genetic_attack_snli(opt, device, model, attack_surface, dataset=opt.dataset, genetic_test_num=500, split="dev")
            out_log="%d epoch with dev gene_adv_acc %.4f" % (epoch,adv_acc)
